...
function AdresaForm({ id=0 }) {
    ...
    const fetchAdresaData = async () => {
        if (id !== 0) { 
            try {
                const response = await fetch(`https://localhost:7043/api/adresa/${id}`);
                if (response.ok) {
    ...
    const handleSubmit = async (e) => {
        ...
        if(validInputs()){
            try {
                ...
                const response = null;
                const text = `ulice=${formData.ulice}&mesto=${formData.mesto}&cislo_popisne=${formData.cislo_popisne}&psc=${formData.psc}&stat=${formData.stat}`
                id===0 ? response=fetch(`https://localhost:7043/api/adresa?${text}`, {method: 'POST', mode: 'cors'}): 
                response =fetch(`https://localhost:7043/api/adresa/${id}?${text}`,{method: 'PUT', mode: 'cors'});
